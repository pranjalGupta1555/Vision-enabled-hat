...
from captions_ref.utils.nn import NN
from captions_ref.utils.coco.coco import COCO
from captions_ref.utils.coco.pycocoevalcap.eval import COCOEvalCap
from captions_ref.utils.misc import ImageLoader, CaptionData, TopN
...
class BaseModel(object):
    def __init__(self, config):
        ...
        self.image_loader = ImageLoader(config.image_mean)
    ...
    def test(self, sess, test_data, vocabulary):
        ...
        for k in tqdm(list(range(test_data.num_batches)), desc='path'):
            ...
            for l in range(test_data.batch_size-fake_cnt):
                ...
                plt.axis('off')
                plt.title(caption)
                image_name=image_name.lstrip('./captions_ref/test/images/')
    ...
    def test_single(self, sess, test_data, vocabulary):
        """ Test the model using any given images. """
        print("Calculating caption for a single image ...")
        config = self.config
        ...
        #                         'prob': scores})
        # results.to_csv(config.test_result_file)
        print("Caption generated as: {}".format(captions))
        return captions[0]
    ...
    def load(self, sess, model_file=None):
        ...
        for v in tqdm(tf.global_variables()):
            if v.name in list(data_dict.keys()):
